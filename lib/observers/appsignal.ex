defmodule AppSignal do
  ...
  def test_connection do
    with {:ok, api_token} <- get_api_token(),
      ...
      url = "https://appsignal.com/api/#{app_id}/markers.json?token=#{api_token}"
  ...
  def to_result(endpoint, method, body) do
    # TODO this actually explodes with decode!/1.
    count =
      body
      |> Poison.decode!()
      |> Map.get("data")
  ...
  def save_api_token!(token), do: LocalStorage.save!("APPSIGNAL_API_TOKEN", token)

  def get_api_token, do: LocalStorage.read("APPSIGNAL_API_TOKEN")

  def save_app_id!(app_id), do: LocalStorage.save!("APPSIGNAL_LIVE_APP_ID", app_id)

  def get_app_id, do: LocalStorage.read("APPSIGNAL_LIVE_APP_ID")
end
